...
import com.example.orderfood.adapter.OrderBusinessFinishAdapter;
...
public class OrderFinishActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<OrderBean> list = OrderDao.getAllOrderByBusinessFinish(account);
        if(list!=null&&list.size()>0){
            OrderBusinessFinishAdapter adapter = new OrderBusinessFinishAdapter(this, list);
        ...
        search.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String s) {
                ...
                if(s.equals("")){
                    List<OrderBean> list = OrderDao.getAllOrderByBusinessFinish(account);
                    if(list!=null&&list.size()>0){
                        OrderBusinessFinishAdapter adapter = new OrderBusinessFinishAdapter(OrderFinishActivity.this, list);
                        listView.setAdapter(adapter);
                    }else{
                        listView.setAdapter(null);
                    }
                    ...
                    List<OrderBean> list = OrderDao.getAllOrderByBusinessFinish(account,s);
                    if(list!=null&&list.size()>0){
                        OrderBusinessFinishAdapter adapter = new OrderBusinessFinishAdapter(OrderFinishActivity.this, list);
                        listView.setAdapter(adapter);
                    }else{
